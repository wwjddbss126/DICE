namespace DICE.Modules.ViewModels.Cloud.Mega
{
	...
	{
		Task<MegaManager.LogonSessionToken> LoginAsync(string email, string password, string mfaKey = null);

		Task<MegaManager.LogonSessionToken> LoginAsync(MegaManager.AuthInfos authInfos);

		Task LoginAsync(MegaManager.LogonSessionToken logonSessionToken);
		...
		Task<MegaManager.AuthInfos> GenerateAuthInfosAsync(string email, string password);

		Task<MegaManager.AuthInfos> GenerateAuthInfosAsync(string email, string password, string mfaKey);
